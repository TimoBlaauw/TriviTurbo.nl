...
// Configuraties
const GHL_WEBHOOK_URL = "https://services.leadconnectorhq.com/hooks/5jh9KpAoe1eQ80Bfj9mP/webhook-trigger/5f751d49-82ea-4e4b-8a88-e5274eb193cf"
...
// Hulpmiddel voor de mailtekst in Web3Forms
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-300">
        ...
        <button onClick={handleClose} className="absolute top-4 right-4 z-10 w-10 h-10 bg-[#072AC8]/10 hover:bg-[#072AC8]/20 rounded-full flex items-center justify-center transition-colors">
          ...
          {step !== "success" && (
            <div className="flex items-center gap-4 mt-4">
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step === "A" ? "bg-[#FCF300] text-[#072AC8]" : "bg-white/20 text-white"}`}>
              ...
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${step === "B" ? "bg-[#FCF300] text-[#072AC8]" : "bg-white/20 text-white"}`}>2</div>
          ...
          {step === "A" && (
            <div className="space-y-5">
              <div>
                ...
                <input type="text" value={formData.firstName} onChange={(e) => updateField("firstName", e.target.value)} placeholder="Bijv. Jan" className={`w-full px-4 py-3 rounded-xl border ${errors.firstName ? "border-red-400" : "border-[#072AC8]/12"} bg-white text-[#072AC8] focus:outline-none focus:ring-2 focus:ring-[#0095FF]/30 transition-all`} />
                {errors.firstName && <p className="text-red-500 text-sm mt-1">{errors.firstName}</p>}
              </div>
              <div>
                <label className="block text-sm font-bold text-[#072AC8] mb-2">E-mail *</label>
                <input type="email" value={formData.email} onChange={(e) => updateField("email", e.target.value)} placeholder="jan@voorbeeld.nl" className={`w-full px-4 py-3 rounded-xl border ${errors.email ? "border-red-400" : "border-[#072AC8]/12"} bg-white text-[#072AC8] focus:outline-none focus:ring-2 focus:ring-[#0095FF]/30 transition-all`} />
                {errors.email && <p className="text-red-500 text-sm mt-1">{errors.email}</p>}
              </div>
              <div>
                <label className="block text-sm font-bold text-[#072AC8] mb-2">Telefoon/WhatsApp *</label>
                <input type="tel" value={formData.phone} onChange={(e) => updateField("phone", e.target.value)} placeholder="+31 6 12345678" className={`w-full px-4 py-3 rounded-xl border ${errors.phone ? "border-red-400" : "border-[#072AC8]/12"} bg-white text-[#072AC8] focus:outline-none focus:ring-2 focus:ring-[#0095FF]/30 transition-all`} />
                {errors.phone && <p className="text-red-500 text-sm mt-1">{errors.phone}</p>}
              </div>
              <div>
                <label className="block text-sm font-bold text-[#072AC8] mb-2">Voorkeurstaal</label>
                <select value={formData.language} onChange={(e) => updateField("language", e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[#072AC8]/12 bg-white text-[#072AC8] focus:outline-none transition-all">
              ...
              <div className="pt-4">
                <Button onClick={() => validateStepA() && setStep("B")} className="w-full bg-gradient-to-r from-[#072AC8] to-[#0095FF] text-white font-bold py-4 rounded-2xl shadow-lg">
          ...
          {step === "B" && (
            <div className="space-y-5">
              ...
              <div className="border-t pt-5 mt-6 space-y-4">
                ...
                <input type="url" placeholder="Huidige website URL" value={formData.currentWebsite} onChange={(e) => updateField("currentWebsite", e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[#072AC8]/12 bg-white" />
                <input type="url" placeholder="Google Business Profile URL" value={formData.googleBusinessProfile} onChange={(e) => updateField("googleBusinessProfile", e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[#072AC8]/12 bg-white" />
                <input type="url" placeholder="Foto's link (Dropbox/Drive)" value={formData.photosLink} onChange={(e) => updateField("photosLink", e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[#072AC8]/12 bg-white" />

                <select value={formData.managesOwnBooking} onChange={(e) => updateField("managesOwnBooking", e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[#072AC8]/12 bg-white">
              ...
              <div className="pt-4">
                <label className="flex items-start gap-3 cursor-pointer">
                  <input type="checkbox" checked={formData.consent} onChange={(e) => updateField("consent", e.target.checked)} className="mt-1 w-5 h-5 rounded text-[#0095FF]" />
                  <span className="text-sm text-[#4b5b8a]">Akkoord: TriviTurbo mag mijn gegevens gebruiken voor het maken van een voorbeeldwebsite. *</span>
                </label>
                {errors.consent && <p className="text-red-500 text-sm mt-2">{errors.consent}</p>}
              </div>

              <div className="flex gap-4 pt-4">
                <Button onClick={() => setStep("A")} variant="outline" className="border-[#072AC8]/12 text-[#072AC8] font-bold px-6 py-4 rounded-2xl">
                  <ChevronLeft className="w-5 h-5 mr-2" /> Terug
                </Button>
                <Button onClick={handleSubmit} disabled={isSubmitting} className="flex-1 bg-gradient-to-r from-[#072AC8] to-[#0095FF] text-white font-bold py-4 rounded-2xl shadow-lg">
